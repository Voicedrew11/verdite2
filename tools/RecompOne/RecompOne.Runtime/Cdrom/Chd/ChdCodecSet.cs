...
internal sealed class ChdCodecSet
{
    ...
    {
        var codec = _compressors[slot];

        if (codec == ChdBig.Tag("cdzl"))
        {
            DecompressCd(source, dest, Zlib);
            return;
        }

        if (codec == ChdBig.Tag("cdlz"))
        {
            DecompressCd(source, dest, Lzma);
            return;
        }

        if (codec == ChdBig.Tag("cdfl"))
        {
            DecompressCdFlac(source, dest);
            return;
        }

        if (codec == ChdBig.Tag("zlib"))
        {
            Zlib(source, 0, source.Length, dest, 0, dest.Length);
            return;
        }

        if (codec == ChdBig.Tag("lzma"))
        {
            Lzma(source, 0, source.Length, dest, 0, dest.Length);
            return;
        }

        throw new NotSupportedException(
            $"chd codec {TagName(codec)} its not supported"); //most should use lzma i believe so shouldnt be a problem
    }
    ...
    {
        var eccBytes = (_frames + 7) / 8;
        var lengthBytes = source.Length < 0x10000 ? 2 : 3;
        var headerBytes = eccBytes + lengthBytes;

        var baseLength = (source[eccBytes] << 8) | source[eccBytes + 1];
        if (lengthBytes > 2) baseLength = (baseLength << 8) | source[eccBytes + 2];

        var dataBytes = _frames * SectorData;
        var subBytes = _frames * SubcodeData;
    ...
    private void DecompressCdFlac(byte[] source, byte[] dest)
    {
        ...
        var consumed = ChdFlac.Decode(source, 0, _scratch, 0, dataBytes);
    ...
    {
        using var input = new MemoryStream(src, srcOffset, srcLength, false);
