...
internal static class ChdEcc
{
        ...
        int dest2)
    {
        ...
        {
            var source = SourceByte(sector, start, offsets[row, i]);
            value1 ^= source;
            value2 ^= source;
            value1 = Low[value1];
        }
    ...
    private static ushort[,] BuildOffsets(int rows, int components, int step, int rowStep)
    {
        ...
        {
            var index = rowStep == 1 ? i / 2 + j * step : (i / 2 * rowStep + j * step) % modulus;
