...
internal static class MenuOrder
{
    public static List<T> Arrange<T>(List<T> entries) where T : MenuEntry
    {
        ...
        while (pending.Count > 0)
        {
            ...
            {
                var entry = pending[i];
                var at = IndexOfKey(placed, entry.AnchorKey!);
                if (at < 0) continue;
                ...
                {
                    at++;
                    while (at < placed.Count && placed[at].AnchorKey == entry.AnchorKey &&
                           !placed[at].AnchorBefore) at++;
                }
    ...
    private static int IndexOfKey<T>(List<T> list, string key) where T : MenuEntry
    {
