...
| `Remaster.*` (`patches/remaster/`) | authored data from a pack: area identity and fingerprint, the working pack (JSON, watched, undo), a material per tile half written into the packet's depth record for the reflection pass, the editor (Shift+E, pauses the world, picks by ray or from the docked map; a pick stops at rock and at a step up) and the `edit`/`select`/`set`/`pack`/`remaster` shell verbs; `snap` hashes the presented picture (`0069`); Video ▸ Enhancements ▸ Remaster packs, with the packs listed under their own heading | off; nothing authored | REMASTER, "Phase 1, the first slice", "Phase 1, the second slice" |
