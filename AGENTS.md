...
- **`KF2_SHELL=1`** — one request per line on TCP 127.0.0.1:27900, one
  ...
  `map [on|off|toggle]`, `goto <x> <y> <z> [yaw [pitch]]`, and the remaster editor's `edit`, `select`, `set`, `pack`, `remaster` (see `help`). The `kf2` MCP server in `mcp/` exposes the same channel.
...
| `Remaster.*` (`patches/remaster/`) | authored data from a pack: area identity and fingerprint, the working pack (JSON, watched, undo), a material per tile half written into the packet's depth record for the reflection pass, the editor (Shift+E, pauses the world, picks by ray or from the docked map) and the `edit`/`select`/`set`/`pack`/`remaster` shell verbs | off; nothing authored | REMASTER, "Phase 1, the first slice" |
