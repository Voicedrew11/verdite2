...
- **`KF2_SHELL=1`** — one request per line on TCP 127.0.0.1:27900, one
  ...
  `map [on|off|toggle]`, `goto <x> <y> <z> [yaw [pitch]]`, and the remaster editor's `edit`, `select`, `set`, `pack`, `remaster`, plus `snap [hash|PATH.png]`, which reads the presented picture back and hashes it (see `help`). The `kf2` MCP server in `mcp/` exposes the same channel.
