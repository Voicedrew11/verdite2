...
|---|---|---|
| **A crossing still shows "a moment of darkness or at least not the correct frame".** Reported from play after the smoothers were rebased at a load. | The load path (`func_80024154`, which `warp` calls too) is clean on every counter: **0 black frames the game asked no tint for**, the tint walking `f`→`0` over the 32-step fade, 1650 ms at 20 Hz with the disc read under 100 ms, `vram fallback 0`, never masked off, no flash, one unprimed frame. `KF2_BLACKPROBE=1`'s `frame` block is the column that would name a wrong frame. **Not yet captured on a walking crossing** — the boundary is a door the shell cannot reach, so it wants a capture from play. Open beside it: whether 1.65 s of fade reads as too long by eye. | "Walking between areas stuttered" in [PATCHES_AND_MODS.md](PATCHES_AND_MODS.md) |
