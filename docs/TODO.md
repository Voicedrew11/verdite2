...
| **The death clock stops short above the tick rate.** Measured, not diagnosed. | The death counter at `0x8019951A` reaches its 65 frames in 3.16 s at `KF2_FPS=20`, which is the documented reference — but at 60 fps it halts at **31** and at 144 fps at **14**, and stays there. Independent of every smoothing switch, of `KF2_LOOPPACING=0` and of `KF2_AUTORELOAD=0` (measured on/off, identical to 0.02 s), so it is not the smoothing patches; the on/off comparison the pose work is checked against is still valid, but the *absolute* 65-in-3.25 s figure in "The bounded mode" only reproduces at the tick rate now. The death sequence is a candidate modal loop; `KF2_LOOPPACING_PROBE=1` through a death is the next reading. | this table |
