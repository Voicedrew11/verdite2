...
| **The smoothing is sometimes dead for a whole session, and the title screen's speed predicts it.** Reported from play; not reproduced. | Fourteen cold starts on the same machine, config and build are *byte-identical*: `165 fps, boundary 3/3 DrawOTag + 3/3 VSync, 7/7 stage(s)`, `smoothing: on`, `objects: on`, and a measured 165.0 fps against 20.0 ticks/s in an area. So the boot path is not where the variance is. The healthy title is **15.0 fps and is not paced by `KF2_FPS` at all** — it is OPEN.EXE's own four-vblank wait on a wall-clock grid, and it reads 15.0 at `KF2_FPS` 20, 60, 165 and 300 alike — so "the title is going at the wrong speed" cannot come from the render rate the settings hold. `KF2_FPS_PROBE=1` was added to close this and **could not**: reported from a broken run, its line is *identical* to a healthy one. That is the probe's own defect rather than a null result — its three fields are two counters and `Extrapolating`, and `Extrapolating` is a pure function of `LogicMode`, `Enabled`, `TargetFps` and `LogicHz`, so it reports configuration and never a reading. It has since been given three health words (`view`/`objects`/`anim`) that do report one. **The second measurement narrows it hard: animation smoothing still works while camera and movement smoothing do not**, in the same session. Everything the three patches share is therefore live — the boundary, `LogicPhase`, `FirstWalkOfTick`, `HookAttach`, the settings view — and the fault is per-patch state in `FrameSmoothing` and `ObjectSmoothing`: `_paired` false (which makes the Video checkbox permanently inert, complaining once on stderr), the saved setting reading off, or `_carriable`/`_primed` never re-primed. The third is the one the asymmetry points at, since `AnimSmoothing` primes per slot while both siblings gate on a single global flag that `OverlayLoadedEvent` — which fires for the `fdat` area modules too — clears. **Needs one health word from a broken run**; note that pinning `KF2_SMOOTH=1`/`KF2_SMOOTH_OBJECTS=1` sets `_onFromEnv` and destroys the evidence for the saved-setting candidate, so leave them unset. | "The rate a session is actually running at" in [PATCHES_AND_MODS.md](PATCHES_AND_MODS.md) |
| **Nobody has looked at area 10.** It loads, draws and walks; whether it is *coherent* is unanswered. | Every reading is a counter: 632.1 map-tile packets a frame, 6 objects and 17 entities live, the player walks and takes damage. What no counter can say is whether the geometry is finished, what the seventeen creatures are, or which model set is right — `RTMD.T` has no entry 10, so the props are drawn either with the previous area's models (the default) or with `KF2_AREA10_RTMD=8`, the one entry in that archive nothing else claims. Warp there and look. | "The eleventh area" in [PATCHES_AND_MODS.md](PATCHES_AND_MODS.md) |
| **`fdat32` is not relocated, so area 10 has no doors, levers or scripted triggers.** | It is linked for `0x80193B38` against a build whose text *and* BSS are laid out differently, and it cannot be loaded at its own base because `GAME.EXE` keeps the billboard table inside that window. Seven of its fifteen external calls now have candidate targets, from matching the **gaps between pairs** rather than the addresses — two order-preserving clusters at `+0x14AC` and `+0x638` — and the data side has three per-region deltas that agree on 5-of-5, 9-of-9 and 9-of-11 offsets. The remaining eight calls fall in gaps the live modules' vocabulary cannot reach. There is also no route yet for a *relocated* image to reach the recompiler, which reads the module's bytes off the disc. | "What a relocation would have to fix" in [RECOMPILATION.md](RECOMPILATION.md) |
