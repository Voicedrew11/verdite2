...
| ~~**The Z-buffer still shows sky through nearby walls.**~~ Answered — the outdoor backdrop is linked at the extreme far end of the ordering table but (census-measured) projects only *mid-depth*, so believing its SZ rejected the distant terrain behind it. `patches/recompone/0025` parks a primitive projecting below the depth its table position predicts (`FarSz × (1 − OtEntry/OtLength) × SkyParkMargin`). Two earlier tries missed: a fixed node band over-captured real geometry, and a near-SZ cut assumed the backdrop was near and never parked it. The residual intermittency was the scale, not the test: `FarSz` was the previous walk's own max and the backdrop fed it, so any near-only window collapsed it under the sky's SZ and the sky came back; it is now the largest depth seen outside a 512-node far band, sticky for the overlay session and reset on each overlay load. A later residual was the *opposite* — walls seen through to the room behind — because the same test over-parked mid-table geometry and the performance fix made parking write a far-plane depth (`ParkedFarSz`) that loses `LEQUAL` to nearer geometry drawn first; gating the park to that far band (`OtEntry < SkyScaleBand`) removes the mechanism, since a far-band park is drawn before anything it could hide. | The clear-at-tail defect (`0016`) was real, fixed and measured, but not the cause. `SkyParkMargin` and the `SkyScaleBand` park gate want calibrating against `KF2_ZBUFFER_PROBE=2` per area; picture re-check pending. | "The clear landed at the tail of the frame" and "The second cause" in [RENDERING.md](RENDERING.md) |
