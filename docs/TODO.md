...
| **The first primitive after the animated textures step costs 0.76 ms.** Measured, not diagnosed. | The frame viewer measured it in both captures that caught the step. `func_8002DC78` uploads ten textures through GP0, 0.3 ms between them. Then the first map-tile quad's send takes 756 µs against the usual 0.2-5 µs, and its batch submit is logged as a depth-mode change. That makes such a frame's GP0 work about 1.1 ms against 0.09 ms. The leading suspect is the driver finishing the uploads' `TexSubImage2D` and blits inside that submit. A GL timer query, or `glFinish` after the uploads, would tell. Also from the same captures: **29 of a frame's 31 batch submits are semi-transparency toggles**, because `GlCore` keys a batch on it. | "Watching a frame being built" in [DEVELOPMENT.md](DEVELOPMENT.md) |
