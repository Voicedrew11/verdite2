...
| **`SettingsRegistry.Extend` can only append, and cannot un-append.** No ordering argument, and no removal API. The port worked around both by taking the Input pane over outright; the gap is still upstream's. Worth an issue. | The pane is fixed; this is the mechanism that made it hard. | "The Input pane is the port's" in [INPUT.md](INPUT.md) |
